...
type InvoiceStatusDto =
  | { $type: 'draft' }
  | { $type: 'sent', sent_on: string }
  | { $type: 'paid', paid_on: string }
  | { $type: 'overdue' };
...
function toInvoiceStatus(dto: InvoiceStatusDto): InvoiceStatus {
  return match(dto)
    .with({ $type: 'draft' }, () => ({ type: 'draft' as const }))
    .with({ $type: 'sent' }, (x) => ({ type: 'sent' as const, sentOn: toLocalDate(x.sent_on) }))
    .with({ $type: 'paid' }, (x) => ({ type: 'paid' as const, paidOn: toLocalDate(x.paid_on) }))
    .with({ $type: 'overdue' }, () => ({ type: 'overdue' as const }))
    .exhaustive();
}