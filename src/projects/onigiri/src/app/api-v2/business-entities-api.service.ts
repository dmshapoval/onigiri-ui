...
import { toBusinessEntity, toBusinessEntityPropertyDto } from './contracts/business-entities';
...
import { BusinessEntityDto } from './contracts/business-entities';
...
export class BusinessesApiService {
  ...
  getDefaultBusinessEntity() {
    return this.#http.get<BusinessEntityDto>(`${environment.onigiriApi}/api/business-entities/default`)
      .pipe(map(toBusinessEntity));
  }

  updateBusinessEntity(entityId: string, data:BusinessEntityData) {
    const payload = Object.entries(data)
      .map(([key, value])=> toBusinessEntityPropertyDto(key as BusinessEntityPropertyKey, value))

    return this.#http.patch<void>(
      `${environment.onigiriApi}/api/business-entities/${entityId}`,
      payload
    );
  }

  // updateBusinessEntity(entityId: string, props: BusinessEntityProperty[]) {
  //   return this.#http.patch<void>(
  //     `${environment.onigiriApi}/api/business-entities/${entityId}`,
  //     props.map(toBusinessEntityPropertyDto)
  //   );
  // }
  ...
  //   return this.#http.put<void>(`${environment.onigiriApi}/api/business-entities/main`, payload);
  // }
